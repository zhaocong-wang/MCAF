...
class VideoResNet(nn.Module):
    ...
    def __init__(self, block, conv_makers, layers,
                 stem, in_channels=4, num_classes=1, msha=False, n_frames_last_layer=2, input_last_layer=(7, 7)):
...
def BabyNet(msha=False, n_frames=16, input_size=(224, 224), in_channels=4, num_classes=2, **kwargs):
    n_frames_last_layer = max(n_frames // 8, 1)
    input_last_layer = (input_size[0] // 16, input_size[1] // 16)
    return _video_resnet(in_channels=in_channels,
...
if __name__ == '__main__':
    ...
    # 模型初始化
    model = BabyNet(n_frames=128, input_size=(128, 128), num_classes=2).to(device)
