...
class _GridAttentionBlockND(nn.Module):
    ...
    def _concatenation(self, x, g):
        ...
        # x:[-1, 256, 128, 16, 16]->[-1, 256, 64, 8, 8]
        theta_x = self.theta(x)
        theta_x_size = theta_x.size()
        # print(theta_x_size, '@@@@@')

        # g:[-1, 512, 128, 8, 8]->[-1, 256, 128, 8, 8]
        phi_g = F.upsample(self.phi(g), size=theta_x_size[2:],
                           mode=self.upsample_mode)  # torch.Size([-1, 256, 64, 8, 8])
...
if __name__ == '__main__':
    GA = GridAttentionBlock3D(in_channels=256, gating_channels=512,
                              inter_channels=256, sub_sample_factor=(2, 2, 2),
                              mode='concatenation')

    x = torch.rand(4, 256, 128, 16, 16)
    g = torch.rand(4, 512, 128, 8, 8)
    out, att = GA(x, g)
    print('out=', out.shape)
    print('att=', att.shape)
